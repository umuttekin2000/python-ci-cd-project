...
from typing import Collection #Collection is used to type hint the urls argument in the main function
...
@click.option('-v', '--verbose', is_flag=True, help='Enable debug logging.')  # Note '-v' instead of "v"
def main(urls: Collection[str], timeout: int, verbose: bool):
    if verbose:
        logging.getLogger().setLevel(logging.DEBUG)
        logging.debug("Verbose mode enabled. Debug logging is active.")
    ...
    if urls:
        ...
        for url, status in results.items():
            if "OK" in status:
                ...
                fg_color = "red"
            click.echo(click.style(f"{url:<40} -> {status}", fg=fg_color)) # Color the output based on status
